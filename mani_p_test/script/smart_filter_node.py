...
class SmartFilterNode(Node):

    def __init__(self):
        super().__init__('smart_filter_node')
        ...
        self.filtered_frame = "shelf_filtered" # New Frame Name
        ...
        self.tf_broadcaster = TransformBroadcaster(self) # Added Broadcaster
        ...
        self.get_logger().info(f'Smart Filter Node Started. Publishing TF: {self.filtered_frame}')
    ...
    def update_shelf(self):
        try:
            ...
            # --- 1. Publish TF ---
            final_roll  = 0.0 # Gravity aligned
            final_pitch = 0.0
            final_yaw   = final_yaw_base
            
            q_tf = self.get_quaternion_from_euler(final_roll, final_pitch, final_yaw)
            ...
            # --- 2. Publish Collision Object (Optional, kept for viz) ---
            if self.cached_mesh_msg is not None:
                ...
                q_mesh = self.get_quaternion_from_euler(
                    final_roll + self.stl_offset_roll, 
                    final_pitch + self.stl_offset_pitch, 
                    final_yaw + self.stl_offset_yaw
                )
                mesh_pose.pose.orientation = Quaternion(x=q_mesh[0], y=q_mesh[1], z=q_mesh[2], w=q_mesh[3])
